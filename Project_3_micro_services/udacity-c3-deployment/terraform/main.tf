...
locals {
  ...
  # az_c             = "${var.aws_region}c" no subnet c in ca-central-1
...
dont need this  since subnet c does not exits
*/

locals {
  # removed subnet c from next line
  all_subnets = [data.aws_subnet.az_a.id, data.aws_subnet.az_b.id]
}
...
resource "aws_instance" "control_plane" {
  #switched next line to 2 since only 2
  count = 2
